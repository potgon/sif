...
import {MonthlyTransactionRows} from "../../api/finances/types.ts";
import {fetchMonthlyTransactionRows} from "../../api/finances/metrics.ts";
...
export default function RecentTransactions({year, month}: Readonly<Props>) {
    const [data, setData] = useState<MonthlyTransactionRows | null>(null)
    const [loading, setLoading] = useState(true)

    useEffect(() => {
        setLoading(true)
        fetchMonthlyTransactionRows(year, month)
            .then(setData)
                            ...
                            {data?.transactions.map((transaction) => (
                                <TableRow key={transaction.id}>
                                    ...
                                    <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                                        <Badge
                                            size="sm"
                                            color={transaction.isRecurring ? "success" : "error"}
                                        >
